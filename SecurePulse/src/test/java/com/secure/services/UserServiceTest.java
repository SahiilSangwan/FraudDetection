...
class UserServiceTest {
    ...
    void updateUser_userNotFound() {
        ...
        CustomException exception = assertThrows(CustomException.class, () -> userService.updateUser(1, user));
    ...
    @Test
    void getUserByEmailAndPassword_fail_incorrectPassword() {
        User user = new User();
        user.setUserId(1);
        user.setPassword("encoded");
        when(userRepository.findByEmail("test@example.com")).thenReturn(Optional.of(user));
        ...
        Optional<User> result = userService.getUserByEmailAndPassword("test@example.com", "wrong", "HDFC");
        assertTrue(result.isEmpty());
    }

    @Test
    void getUserByEmailAndPassword_fail_userNotFound() {
        when(userRepository.findByEmail("wrong@example.com")).thenReturn(Optional.empty());
        Optional<User> result = userService.getUserByEmailAndPassword("wrong@example.com", "raw", "HDFC");
        assertTrue(result.isEmpty());
    }
    ...
    @Test
    void setUserMpin_invalidLength() {
        CustomException exception = assertThrows(CustomException.class, () -> userService.setUserMpin("test@example.com", "12345"));
    ...
    @Test
    void updateUserMpin_invalidLength() {
        CustomException exception = assertThrows(CustomException.class, () -> userService.updateUserMpin("test@example.com", "1234"));
        assertEquals("MPIN must be 6 digits", exception.getMessage());
    }

    @Test
    void updateUserMpin_mpinNotSet() {
        User user = new User();
        user.setMpin(null);

        when(userRepository.findByEmail("test@example.com")).thenReturn(Optional.of(user));

        CustomException exception = assertThrows(CustomException.class, () -> userService.updateUserMpin("test@example.com", "654321"));
        assertEquals("MPIN not set. Please set MPIN first", exception.getMessage());
    }
    ...
    @Test
    void verifyMpin_userNotFound() {
        when(userRepository.findByEmail("test@example.com")).thenReturn(Optional.empty());

        CustomException exception = assertThrows(CustomException.class, () -> userService.verifyMpin("test@example.com", "123456"));
        assertEquals("User not found", exception.getMessage());
    }

    @Test
    void verifyMpin_mpinNotSet() {
        User user = new User();
        user.setMpin(null);
        ...
        CustomException exception = assertThrows(CustomException.class, () -> userService.verifyMpin("test@example.com", "123456"));
        assertEquals("MPIN not set for this user", exception.getMessage());
    }

    @Test
    void verifyMpin_incorrectMpin() {
        User user = new User();
        user.setMpin("encoded");
        ...
        CustomException exception = assertThrows(CustomException.class, () -> userService.verifyMpin("test@example.com", "wrong"));
    ...
    @Test
    void verifyMpinOtp_invalidOtp() {
        User user = new User();
        user.setMpin("encoded");

        when(userRepository.findByEmail("test@example.com")).thenReturn(Optional.of(user));
        when(passwordEncoder.matches("123456", "encoded")).thenReturn(true);
        when(otpProvider.validateOtp("test@example.com", "000000")).thenReturn(false);

        CustomException exception = assertThrows(CustomException.class, () -> userService.verifyMpinOtp("test@example.com", "000000", "123456"));
